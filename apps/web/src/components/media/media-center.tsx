...
function OwnerAvatars() {
  ...
  const { data, error } = useQuery({ queryKey: ["media", "people", ctx?.user_id], enabled: !!ctx?.is_owner, queryFn: async () => {
    const users = [];
    for (let page = 1; ; page++) {
      const result = await browserApi<{ items: { id: string; full_name: string | null; email: string }[]; pagination: { total_pages: number } }>(`/users?page=${page}&page_size=100&status=active`);
      users.push(...result.items);
      if (page >= result.pagination.total_pages) break;
    }
    return users;
  } });
  return <section className="mb-6"><label className="grid gap-2 text-sm"><span>{locale === "id-ID" ? "Kelola foto pengguna sekolah" : "Manage school user photos"}</span><select className="ose-control" value={userId} onChange={e => setUserId(e.target.value)}><option value="">{locale === "id-ID" ? "Pilih pengguna" : "Choose user"}</option>{data?.map(user => <option key={user.id} value={user.id}>{user.full_name || user.email}</option>)}</select></label>{error && <p role="alert">{locale === "id-ID" ? "Daftar pengguna gagal dimuat." : "Unable to load users."}</p>}{userId && <FixedMedia key={userId} kind="avatar" userId={userId}/>}</section>;
}
function MediaLibrary({ category }: { category: "gallery" | "learning" }) {
  ...
  const upload = category === "gallery" ? ctx.is_owner : ctx.can_upload_learning;
