...
export function MediaUpload({ bucket, prefix, fixedName, onUploaded }: { bucket: MediaBucket; prefix: string; fixedName?: string; onUploaded?: () => void }) {
  ...
  return <div><input ref={input} type="file" className="sr-only" tabIndex={-1} aria-label={id ? "Pilih berkas" : "Choose file"} accept={bucket === "tenant-learning" ? "image/png,image/jpeg,image/webp,application/pdf,video/mp4,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" : "image/png,image/jpeg,image/webp"} disabled={busy} onChange={e => void upload(e.target.files?.[0])}/><div className="ose-media-actions"><button type="button" className="ose-file-label" disabled={busy} onClick={() => input.current?.click()}><Upload size={16}/>{busy ? (id ? "Mengunggah…" : "Uploading…") : (id ? "Unggah / ganti berkas" : "Upload / replace file")}</button><span className="text-xs text-muted-foreground">{bucket === "tenant-learning" ? "PNG, JPG, WebP, PDF, DOC, DOCX, MP4 · 20 MB" : "PNG, JPG, WebP · 5 MB"}</span></div><p role="status" className="mt-2 text-sm">{message}</p></div>;
}
...
export function FixedMedia({ kind, userId }: { kind: "avatar" | "school" | "platform"; userId?: string }) {
  ...
  const allowed = ctx && (kind === "platform" ? ctx.is_platform_admin : kind === "school" ? ctx.can_manage_media : ctx.can_manage_media || !userId || userId === ctx.user_id);
