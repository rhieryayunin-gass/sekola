...
function Workspace({ context }: { context: ConnectContext }) {
  const { locale } = useTranslations(); const id = locale === "id-ID";
  const params = useSearchParams(); const select = useCenter(s=>s.select); const cache = useQueryClient();
  const selected = useCenter(s=>s.conversation);
  ...
  const open = (conversation: string) => { select(conversation); setCreating(false); };
  ...
  return <div className="oc-workspace" data-chat-open={!!selected}>
    ...
    {creating && <NewConversation canManage={context.can_manage} platformOwner={context.platform_owner} onClose={() => setCreating(false)} onCreated={open}/>}
