...
export function NewConversation({ canManage, platformOwner = false, onClose, onCreated, initialKind = "DIRECT" }: { canManage: boolean; platformOwner?: boolean; onClose: () => void; onCreated: (id: string) => void; initialKind?: string }) {
  ...
  const contacts = useQuery({ queryKey: ["connect", "contacts", kind, deferred], queryFn: () => connectRpc<Contact[]>("contacts", { search: deferred, for_group: kind === "GROUP" }), enabled: ["DIRECT", "GROUP"].includes(kind) });
  ...
  return <ConnectDialog title={id ? "Mulai percakapan" : "Start a conversation"} onClose={onClose}>
    ...
    {["DIRECT", "GROUP"].includes(kind) ? <form onSubmit={submit}>
      ...
      <label className="oc-search"><Search size={18}/><input value={search} onChange={e => setSearch(e.target.value)} placeholder={platformOwner && kind === "DIRECT" ? (id ? "Cari nama atau sekolah" : "Search names or schools") : (id ? "Cari nama di sekolah Anda" : "Search your school directory")} aria-label={id ? "Cari kontak" : "Search contacts"} maxLength={80}/></label>
      {!canManage && <p className="oc-hint">{id ? "Kontak tersedia sesuai peran Anda di sekolah." : "Available contacts follow your school role."}</p>}
