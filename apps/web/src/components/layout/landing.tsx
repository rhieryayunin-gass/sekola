...
import { ArrowDown, ArrowUpRight, BookOpen, Check, GraduationCap, Mail, MapPin, Phone, Users, X } from "lucide-react";
...
function ModuleDetails({ module, close }: { module: MarketingModule; close: () => void }) {
  ...
  return <dialog ref={dialog} className="ose-module-dialog" aria-labelledby="module-title" aria-describedby="module-description" onClose={close} onClick={event => { if (event.target === dialog.current) { const rect = dialog.current.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) close(); } }}>
    ...
    <div className="ose-dialog-art">{module.id==="o-connect"?<ConnectIllustration/>:<Image src={`/illustrations/${module.id}.webp`} alt="" width={1536} height={1024} sizes="(max-width: 760px) 90vw, 400px"/>}</div>
...
export function Landing() {
  ...
  return <main id="ose-main" className="ose-marketing">
    <section className="ose-hero ose-hero-new"><div className="ose-hero-text"><h1>{copy.title}<br/><span>{copy.accent}</span></h1><p className="ose-hero-copy">{copy.intro}</p><div className="school-hero-actions"><a className="ose-contact-link" href="#contact">{copy.contact}<ArrowDown size={18}/></a><Link className="ose-contact-link" href="/assessment">{locale === "id-ID" ? "Cek kesiapan digital sekolah" : "Assess your school's digital readiness"}<ArrowUpRight size={18}/></Link></div></div><div className="ose-school-scene"><Image src="/illustrations/hero.webp" alt={locale === "id-ID" ? "Guru dan siswa terhubung dalam lingkungan sekolah digital" : "A teacher and students connected in a digital school"} width={1536} height={1024} preload sizes="(max-width: 760px) 100vw, 55vw"/></div></section>
    <section className="ose-ecosystem" id="ecosystem"><div className="ose-section-heading"><div><p className="ose-eyebrow">OSEKOLA ECOSYSTEM</p><h2>{copy.modules}</h2></div><span className="ose-section-hint">{copy.moduleHint}</span></div><div className="ose-product-grid">{marketingModules.map(module => { const content = locale === "id-ID" ? module.idCopy : module.enCopy; return <button type="button" key={module.id} className="ose-product-card glass-panel" aria-haspopup="dialog" onClick={() => setSelected(module)}><BrandIcon name={module.icon}/><h3>{module.name}</h3><p>{content[1]}</p><span className="ose-card-action">{copy.details}<ArrowUpRight size={18}/></span></button>; })}</div></section>
    <CurriculumSupport/><Pricing/>
    <section className="ose-people-close"><p className="ose-eyebrow">TEACHER · STUDENT · PARENT</p><h2>{copy.closing}</h2><p>{copy.closingCopy}</p><div aria-hidden="true" className="ose-people-symbols"><div className="ose-person-symbol"><GraduationCap/></div><i/><div className="ose-person-symbol"><BookOpen/></div><i/><div className="ose-person-symbol"><Users/></div></div></section>
