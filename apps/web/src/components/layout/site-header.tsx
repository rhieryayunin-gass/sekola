...
export function SiteHeader({ theme }: { theme: "light" | "dark" }) {
  ...
  return <header className="ose-header">
    ...
    <div className="ose-nav">
      ...
      <div className="ose-preferences"><LanguageSwitcher /><ThemeSwitcher initial={theme}/>{app && <details key={path} className="ose-menu ose-account"><summary aria-label={t("account")}><span className="ose-avatar">{role.slice(0,1) || "•"}</span></summary><div className="ose-account-menu"><span className="ose-eyebrow">{role || t("account")}</span><Link href="/dashboard/profile">{t("profile")}</Link><LogoutButton /></div></details>}</div>
    </div>
