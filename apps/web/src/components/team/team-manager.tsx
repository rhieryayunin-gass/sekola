  ...
  return (
    <section className="mt-6 grid gap-5">
      ...
      <Modal isOpen={taskModal} onClose={() => setTaskModal(false)} title="Create task" description="The task starts in Backlog and can be dragged across the board.">
        <form className="space-y-3" onSubmit={submitTask}><Input name="title" label="Summary" required /><label className="grid gap-1.5 text-sm font-medium">Description<textarea name="description" className="min-h-24 rounded-xl border border-border bg-surface p-3" /></label><Select name="priority" label="Priority" defaultValue="MEDIUM">{["LOWEST", "LOW", "MEDIUM", "HIGH", "HIGHEST"].map((priority) => <option key={priority}>{priority}</option>)}</Select><Select name="assignee_user_id" label="Assignee"><option value="">Unassigned</option>{(members.data ?? []).map((member) => <option key={member.id} value={member.user_id}>{person(member.user)}</option>)}</Select><Input name="due_date" type="date" label="Due date" />{error && <p className="text-sm text-danger">{error}</p>}<Button type="submit" disabled={createTask.isPending}>Create task</Button></form>
      </Modal>
