    ...
    <section className="mt-6 grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)]">
      <Card className="h-fit p-4">
        <div className="flex items-center justify-between gap-3">
          <div><p className="text-xs font-bold uppercase tracking-wider text-muted">Projects</p><h1 className="mt-1 text-xl font-black">Team+</h1></div>
          <Button size="sm" onClick={() => setProjectModal(true)}>New</Button>
        </div>
      ...
      <div className="min-w-0">
        ...
        {activeProject ? (
          <>
            ...
            {view === "activity" && <Card className="mt-5"><h3 className="text-lg font-bold">Project activity</h3><div className="mt-4 space-y-3">{activity.data?.map((item) => <div key={item.id} className="border-b border-border pb-3"><p className="font-semibold">{item.activity_type.replaceAll("_", " ")}</p><p className="text-sm text-muted">{person(item.actor)} · {new Date(item.created_at).toLocaleString("id-ID")}</p></div>)}{!activity.data?.length && <EmptyState title="No activity yet" description="Task, comment, and finance changes will appear here." />}</div></Card>}
        ...
        ) : <Card><EmptyState title="Create your first Team+ project" description="Set up a project, add members, and manage delivery on a Kanban board." /><div className="mt-4 text-center"><Button onClick={() => setProjectModal(true)}>Create project</Button></div></Card>}
      </div>

      <Modal isOpen={projectModal} onClose={() => setProjectModal(false)} title="Create Team+ project" description="Start a Jira-style workspace for a school initiative.">
        <form className="space-y-3" onSubmit={submitProject}><Input name="code" label="Project key" placeholder="DIGITAL" required /><Input name="name" label="Project name" required /><Input name="description" label="Description" /><Select name="owner_user_id" label="Project owner" defaultValue={me.data?.id} required><option value="">Select owner</option>{availableUsers.map((user) => <option key={user.id} value={user.id}>{person(user)}</option>)}</Select>{error && <p className="text-sm text-danger">{error}</p>}<Button type="submit" disabled={createProject.isPending}>Create project</Button></form>
      </Modal>

      <Modal isOpen={taskModal} onClose={() => setTaskModal(false)} title="Create task" description="The task starts in Backlog and can be dragged across the board.">
        <form className="space-y-3" onSubmit={submitTask}><Input name="title" label="Summary" required /><label className="grid gap-1.5 text-sm font-medium">Description<textarea name="description" className="min-h-24 rounded-xl border border-border bg-white/80 p-3" /></label><Select name="priority" label="Priority" defaultValue="MEDIUM">{["LOWEST", "LOW", "MEDIUM", "HIGH", "HIGHEST"].map((priority) => <option key={priority}>{priority}</option>)}</Select><Select name="assignee_user_id" label="Assignee"><option value="">Unassigned</option>{(members.data ?? []).map((member) => <option key={member.id} value={member.user_id}>{person(member.user)}</option>)}</Select><Input name="due_date" type="date" label="Due date" />{error && <p className="text-sm text-danger">{error}</p>}<Button type="submit" disabled={createTask.isPending}>Create task</Button></form>
      </Modal>

      <Modal isOpen={Boolean(selectedTask)} onClose={() => setSelectedTask(undefined)} title={selectedTask ? `${activeProject?.code}-${selectedTask.task_number} · ${selectedTask.title}` : "Task details"} description={selectedTask?.description || "No description"}>
        {selectedTask && <div><div className="flex flex-wrap gap-2"><Priority value={selectedTask.priority} /><Badge tone="info">{selectedTask.status.replaceAll("_", " ")}</Badge><Badge>{person(selectedTask.assignee)}</Badge></div><h4 className="mt-6 font-bold">Comments</h4><div className="mt-3 max-h-52 space-y-3 overflow-y-auto">{comments.data?.map((comment) => <div key={comment.id} className="rounded-xl bg-white/60 p-3"><p className="text-sm">{comment.body}</p><p className="mt-1 text-xs text-muted">{person(comment.author)} · {new Date(comment.created_at).toLocaleString("id-ID")}</p></div>)}</div><form className="mt-3 flex gap-2" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const body = String(new FormData(form).get("body") ?? "").trim(); if (body) addComment.mutate({ body }, { onSuccess: () => form.reset() }); }}><Input name="body" placeholder="Add a comment" required /><Button type="submit" disabled={addComment.isPending}>Comment</Button></form></div>}
      </Modal>
    </section>
  );
}
...
function SettingsPanel({ settings, members, users, onAddMember, onSave }: { settings?: Settings; members: Member[]; users: User[]; onAddMember: (body: object) => void; onSave: (body: object) => void }) {
  return <div className="mt-5 grid gap-5 lg:grid-cols-2"><Card><h3 className="text-lg font-bold">Project members</h3><div className="mt-4 space-y-2">{members.map((member) => <div key={member.id} className="flex justify-between rounded-xl bg-white/60 p-3"><span>{person(member.user)}</span><Badge>{member.member_role}</Badge></div>)}</div><form className="mt-4 grid gap-3" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onAddMember({ user_id: form.get("user_id"), member_role: form.get("member_role") }); }}><Select name="user_id" label="Add user" required><option value="">Select user</option>{users.map((user) => <option key={user.id} value={user.id}>{person(user)}</option>)}</Select><Select name="member_role" label="Role" defaultValue="MEMBER">{["MANAGER", "MEMBER", "VIEWER"].map((role) => <option key={role}>{role}</option>)}</Select><Button type="submit">Add member</Button></form></Card><Card><h3 className="text-lg font-bold">Project settings</h3>{settings && <form className="mt-4 space-y-4" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onSave({ visibility: form.get("visibility"), notifications_enabled: form.get("notifications_enabled") === "on", allow_member_comments: form.get("allow_member_comments") === "on" }); }}><Select name="visibility" label="Visibility" defaultValue={settings.visibility}><option value="PROJECT">Project members</option><option value="TENANT">Everyone in tenant</option></Select><label className="flex gap-2 text-sm"><input name="notifications_enabled" type="checkbox" defaultChecked={settings.notifications_enabled} /> Notifications enabled</label><label className="flex gap-2 text-sm"><input name="allow_member_comments" type="checkbox" defaultChecked={settings.allow_member_comments} /> Members may comment</label><Button type="submit">Save settings</Button></form>}</Card></div>;
}

function FinancePanel({ data, onInvoice, onPayment }: { data?: Finance; onInvoice: (body: object) => void; onPayment: (body: object) => void }) {
  return <div className="mt-5 space-y-5"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Invoiced", data?.summary.invoiced_amount], ["Paid", data?.summary.paid_amount], ["Outstanding", data?.summary.outstanding_amount], ["Open invoices", data?.summary.outstanding_invoices]].map(([label, value]) => <Card key={String(label)}><p className="text-sm text-muted">{label}</p><p className="mt-2 text-2xl font-black">{label === "Open invoices" ? Number(value ?? 0) : money(Number(value ?? 0))}</p></Card>)}</div><div className="grid gap-5 lg:grid-cols-2"><Card><h3 className="font-bold">Project invoices</h3><div className="mt-3 space-y-2">{data?.invoices.map((invoice) => <div key={invoice.id} className="rounded-xl bg-white/60 p-3"><div className="flex justify-between"><span className="font-semibold">{invoice.invoice_number}</span><Badge>{invoice.status}</Badge></div><p className="mt-1 text-sm text-muted">{money(invoice.amount)} · due {invoice.due_date}</p></div>)}</div><form className="mt-4 grid gap-3" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onInvoice({ invoice_number: form.get("invoice_number"), description: form.get("description") || undefined, amount: Number(form.get("amount")), due_date: form.get("due_date") }); }}><Input name="invoice_number" label="Invoice number" required /><Input name="description" label="Description" /><Input name="amount" type="number" min="0.01" step="0.01" label="Amount" required /><Input name="due_date" type="date" label="Due date" required /><Button type="submit">Create invoice</Button></form></Card><Card><h3 className="font-bold">Payments</h3><div className="mt-3 space-y-2">{data?.payments.map((payment) => <div key={payment.id} className="rounded-xl bg-white/60 p-3"><div className="flex justify-between"><span className="font-semibold">{payment.receipt_number}</span><Badge tone="success">{payment.status}</Badge></div><p className="mt-1 text-sm text-muted">{money(payment.amount)}</p></div>)}</div><form className="mt-4 grid gap-3" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onPayment({ project_invoice_id: form.get("project_invoice_id"), receipt_number: form.get("receipt_number"), amount: Number(form.get("amount")), status: "CONFIRMED" }); }}><Select name="project_invoice_id" label="Invoice" required><option value="">Select invoice</option>{data?.invoices.filter((invoice) => !["PAID", "VOID"].includes(invoice.status)).map((invoice) => <option key={invoice.id} value={invoice.id}>{invoice.invoice_number} · {money(invoice.amount)}</option>)}</Select><Input name="receipt_number" label="Receipt number" required /><Input name="amount" type="number" min="0.01" step="0.01" label="Amount" required /><Button type="submit">Record payment</Button></form></Card></div></div>;
}
