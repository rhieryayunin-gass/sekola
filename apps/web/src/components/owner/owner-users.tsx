...
import { StaffTeacherRoles } from "./part5-actions";
...
function UserEditor({ action, close }: { action: Action; close: () => void }) {
  ...
  const { kind, row } = action; const self = actor === row?.id; const dualRole = Array.isArray(row?.roles) && row.roles.length > 1;
  const fields: OwnerField[] = kind === "password" ? [{ key: "password", label: "ownerNewPassword", type: "password", required: true }] : [
    ...
    ...(!self && !dualRole ? [{ key: "role", label: "ownerRole" as const, required: true, options: roles.map(r => ({ value: r, label: r })) }] : []), ...(!row ? [{ key: "password", label: "ownerNewPassword" as const, type: "password", required: true }] : []),
  ];
