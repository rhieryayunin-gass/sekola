...
export function ProfileForm({ initial }: { initial: Profile }) {const copy=useUiText();
  ...
  return <form className="grid gap-4" onSubmit={submit}>
    <Input label={copy("Full name")} value={form.full_name ?? ""} onChange={(e)=>set("full_name",e.target.value)} required minLength={2} />
    <Input label={copy("Avatar URL")} type="url" value={form.avatar_url ?? ""} onChange={(e)=>set("avatar_url",e.target.value)} placeholder="https://…" />
    <Input label={copy("Phone")} value={form.phone ?? ""} onChange={(e)=>set("phone",e.target.value)} />
