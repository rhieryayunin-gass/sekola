...
export function RoleOverview(){
 const {locale,t}=useTranslations();const school=useSchoolContext();const [selectedSchool,setSelectedSchool]=useState("");const foundation=school.data?.foundation_principal;const activeSchool=school.data?.foundation_schools?.find(s=>s.id===selectedSchool)?.id||school.data?.foundation_schools?.[0]?.id;const id=locale==="id-ID";const context=usePermissionStore(s=>s.context);const user=useAuthStore(s=>s.user);const role=primaryRole(context);const q=useQuery({queryKey:["role-dashboard",context?.userId],enabled:!!context,queryFn:()=>schoolRpc<Dashboard>("school_role_dashboard"),refetchInterval:60000});const d=q.data;const approvalCount=useApprovalCount();const pending=approvalCount.data??0;
 ...
 return <section className="owner-workspace"><header className="owner-banner p5-role-banner"><div><p className="ose-eyebrow"><BrandIcon name={role} size={22}/> OSEKOLA · {role}</p><h1>{t("hello")}, {user?.user_metadata.full_name||user?.email?.split("@")[0]||t("account")}.</h1><p>{t(intro[role]??"workspaceIntro")}</p><LeaveRequest/></div><Image src={`/illustrations/${role.toLowerCase()}.png`} alt="" width={1536} height={1024} sizes="(max-width: 640px) 260px, 440px" priority/></header>{role==="PRINCIPAL"&&!foundation&&pending>0&&<div className="p9-approval-beacon"><Link className="ose-cta" href="/dashboard/approval">Need Approval <span>{pending}</span> →</Link></div>}{foundation&&<nav className="p10-school-switcher" aria-label={id?"Sekolah yayasan":"Foundation schools"}>{school.data?.foundation_schools?.map(s=><button key={s.id} aria-pressed={s.id===activeSchool} onClick={()=>setSelectedSchool(s.id)}><span/>{s.name}<small>{s.group}</small></button>)}{!school.data?.foundation_schools?.length&&<p>{id?"Owner belum menugaskan sekolah aktif dalam yayasan Anda.":"No active schools have been assigned to your foundation."}</p>}</nav>}<SchoolSetupLink/>{q.isError?<p role="alert">{t("loadError")}<Button onClick={()=>void q.refetch()}>{t("retry")}</Button></p>:<>{role==="PRINCIPAL"?foundation&&!activeSchool?null:<PrincipalDashboard key={activeSchool??"home"} tenantId={foundation?activeSchool:undefined}/>:role==="STAFF"?<StaffShortcuts/>:<div className="owner-metrics owner-metrics-four">{metrics.map(([label,v])=><article className="owner-metric glass-panel" key={label}><span>{label}</span><strong>{v??"…"}</strong><small>{d?.school}</small></article>)}</div>}</>}{!foundation&&role!=="STUDENT"&&<SchoolGallery preview/>}{foundation?null:role==="PARENT"?d?.children.map(child=><PersonalAttendance key={child.id} studentId={child.id} name={child.name}/>):<PersonalAttendance/>}</section>;
}
