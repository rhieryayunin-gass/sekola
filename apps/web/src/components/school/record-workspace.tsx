...
function ChoiceField({ field, value, values, onChange, disabled }: { disabled?: boolean; field: Field; value: string; values: Record<string, string>; onChange: (v: string) => void }) {
  ...
  return <><Select name={field.key} aria-label={field.label} value={value} onChange={e => onChange(e.target.value)} required={!field.optional} disabled={disabled || catalog.isLoading}><option value="">{catalog.isLoading ? "…" : field.label}</option>{choices.map(row => <option key={row.id} value={row.id}>{rowName(row)}</option>)}</Select>{catalog.isError && <button type="button" className="ose-link" onClick={() => void catalog.refetch()}>Muat ulang pilihan</button>}</>;
}
export function RecordForm({ spec, row, pending, onSave, onClose, defaults = {} }: { defaults?: Record<string, unknown>; spec: ResourceSpec; row?: SchoolRow; pending: boolean; onSave: (payload: Record<string, unknown>) => void; onClose: () => void }) {
  const { locale } = useTranslations(); const id = locale === "id-ID";
  const initial = { ...defaults, ...row };
  const [values, setValues] = useState<Record<string, string>>(() => Object.fromEntries(spec.fields.map(f => [f.key, initial[f.key] == null ? (f.type === "checkbox" ? String(f.key === "is_active") : "") : f.type === "datetime-local" ? localInput(String(initial[f.key])) : f.type === "lines" && Array.isArray(initial[f.key]) ? (initial[f.key] as string[]).join("\n") : String(initial[f.key])])));
...
export function SchoolHierarchy() { const context = usePermissionStore(s => s.context); return <nav className="school-hierarchy" aria-label="School hierarchy">{[["OSEKOLA", "/dashboard"], ["Tenant", "/dashboard/core"], ["Academic year", "/dashboard/academic"], ["Semester", "/dashboard/academic"], ["Classroom", "/dashboard/academic"], ["Subject", "/dashboard/academic"], ["Courses", "/dashboard/learning"], ["Lessons", "/dashboard/learning"], ["Assignment", "/dashboard/learning"], ["Submission", "/dashboard/learning"]].filter(([, href]) => (href !== "/dashboard/academic" || moduleRoleAllowed("academic", context)) && (href !== "/dashboard/learning" || moduleRoleAllowed("learning", context))).map(([label, href]) => <Link key={label} href={href}>{label}<span>›</span></Link>)}</nav>; }
