export type MediaContext = { user_id: string; tenant_id: string | null; tenant_name: string | null; is_owner: boolean; can_manage_media: boolean; is_platform_admin: boolean; can_upload_learning: boolean };
...
export function validateMedia(file: File, bucket: MediaBucket) {
  const allowed = bucket === "tenant-learning" ? [...imageTypes, "application/pdf", "video/mp4", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"] : imageTypes;
