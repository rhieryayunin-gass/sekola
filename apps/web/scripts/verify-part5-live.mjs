...
try{
 ...
 for(const actor of users.slice(0,6)){
  ...
  if([principal,staff,teacher,parent].includes(actor)){await page.goto('https://osekola.com/dashboard/gallery');await page.locator('.p5-gallery-photo').first().waitFor();await page.screenshot({path:output+'/'+actor.role.toLowerCase()+'-gallery.png',fullPage:true});}
