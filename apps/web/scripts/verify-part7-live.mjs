...
import {verifyPart11} from './verify-part11-live.mjs';
import {verifyPart10} from './verify-part10-live.mjs';
import {verifyPart9} from './verify-part9-live.mjs';
import {verifyPart8} from './verify-part8-live.mjs';
...
try {
  ...
  await rpc(owner, 'school_owner_save', { kind: 'tenant', record_id: tenant_id, payload: { name: `Part 7 verification ${run} 0`, address: 'Synthetic school address', contact_email: 'office@demo.osekola.test', timezone: 'Asia/Jakarta', locale: 'en-US' } });
  ...
  const staffPage = await open(staff, '/dashboard'); await staffPage.getByRole('link', { name: 'Build / Update / Upgrade Your Digital School', exact: true }).click(); await staffPage.locator('[data-testid="school-setup"]').waitFor();
  await picture(staffPage, 'setup-desktop'); await picture(staffPage, 'setup-mobile', true);
  const removed = await staffPage.request.post('https://osekola.com/api/school/setup/suggest', { headers: { Origin: 'https://osekola.com' }, data: {} });
  assert.equal(removed.status(), 410); record('Removed setup assistant rejects generation');

  await verifyPart8({browser,staffPage,owner,staff,student,parent,peer,ids,tenants,run,program,stages,curriculumSave,rpc,denied,record,picture,open,applicationIds,admin});
  ...
  await verifyPart9({browser,users,staff,teacher,student,parent,peerParent,foreign,owner,ids,tenants,run,admin,rpc,denied,record,picture,open});
  await verifyPart10({users,owner,staff,teacher,student,parent,foreign,ids,tenants,run,admin,rpc,denied,record,picture,open,applicationIds});
  await verifyPart11({users,owner,staff,teacher,student,parent,peer,peerParent,foreign,ids,tenants,run,admin,rpc,denied,record,picture,open,output});
  success = true;
} catch (error) {
  ...
  // Break only this run's approval/chat reference cycle before deleting fixtures.
  for(const [table,patch] of [['leave_requests',{chat_message_id:null,approval_request_id:null}],['oconnect_messages',{leave_request_id:null}],['school_user_changes',{approval_request_id:null}]]){try{ok(await admin.from(table).update(patch).in('tenant_id',tenants),'Cleanup references '+table);}catch(e){failures.push(e.message);}}
  const tables = ['payments','student_bills','school_billing_plans','finance_categories','finance_accounts','finance_periods','school_asset_reservations','school_timetable','room_bookings','team_task_comments','team_project_payments','team_project_invoices','team_project_activity','school_project_committees','team_tasks','team_project_members','team_project_settings','team_projects','school_user_credentials','school_user_attendance','attendance_records','school_user_changes','approval_steps','leave_requests','approval_requests','oconnect_messages','oconnect_members','oconnect_conversations','school_cash_entries','school_settlement_accounts', 'school_qualification_results', 'school_setup_suggestions', 'school_setup', 'school_submission_versions', 'school_submission_workflow', 'school_learning_progress', 'school_lesson_versions', 'school_curriculum_evidence', 'school_curriculum_alignments', 'school_assessment_participants', 'school_assessment_designs', 'school_exam_attempts', 'exam_results', 'exam_sessions', 'school_exam_items', 'submissions', 'assignments', 'lessons', 'school_learning_units', 'school_learning_spaces', 'school_curriculum_course_links', 'school_curriculum_enrollments', 'school_curriculum_outcomes', 'school_curriculum_scales', 'school_curriculum_subjects', 'school_curriculum_stages', 'school_curriculum_programs', 'school_ai_generations', 'school_question_items', 'school_question_sets', 'exams', 'courses', 'school_guardians', 'student_assignments', 'teacher_assignments', 'teachers', 'students', 'classrooms', 'subjects', 'semesters', 'academic_years', 'calendars','school_assets','rooms', 'notifications', 'audit_logs', 'school_settings'];
  // Remove child activity references within this run before deleting parent activities.
  try{ok(await admin.from('team_tasks').update({parent_task_id:null}).in('tenant_id',tenants),'Cleanup nested activities');}catch(e){failures.push(e.message);}
  ...
  for(const id of applicationIds){try{ok(await admin.from('school_partner_applications').delete().eq('id',id),'Cleanup fixture application');}catch(e){failures.push(e.message);}}
