...
do $$ declare p jsonb; a uuid; b uuid; sa uuid; sb uuid; suba uuid; subb uuid; cp uuid; tp uuid; ao uuid; scalea uuid; scaleb uuid; evidence uuid; bank uuid; course uuid:='d3000000-0000-4000-8000-000000000007'; result jsonb; begin
 ...
 scaleb:=(public.school_curriculum_save('scales',jsonb_build_object('program_id',b,'name','School rubric v1','kind','RUBRIC','minimum',0,'maximum',8,'criteria','School assessment rubric; no external grade conversion'))->>'id')::uuid;
 ...
 result:=public.school_ai_reserve(gen_random_uuid(),bank,1);
 if jsonb_array_length(result->'curriculum_alignment')<>1 or result->'curriculum_alignment'->0->>'code'<>'TP-C-1' or (result->'curriculum_alignment'->0) ?| array['student_id','feedback','score','full_name'] then raise exception 'AI curriculum alignment missing or contains student data'; end if;
 ...
 evidence:=(public.school_curriculum_save('evidence',jsonb_build_object('course_id',course,'outcome_id',ao,'student_id','d3000000-0000-4000-8000-000000000004','scale_id',scaleb,'score',6,'feedback','Published rubric evidence','status','PUBLISHED'))->>'id')::uuid;
 ...
 result:=public.school_curriculum_learning(course);
 if not exists(select 1 from jsonb_array_elements(result->'evidence') e where e->>'id'=evidence::text and e->'assessment_context'->'program'->>'version'='2025–2027' and e->'assessment_context'->'scale'->>'maximum'='8.00') then raise exception 'Assessment version snapshot missing'; end if;
end $$;
